...
export const checkAndAwardBadges = async (playerId, actionType, additionalData = {}) => {
  try {
    const playerRef = doc(firestore, 'players', playerId);
    const playerDoc = await getDoc(playerRef);
    ...
    const eligibleBadges = checkEligibleBadges(playerData, actionType, additionalData);
