...
export async function reassignTargets(killerUid, victimId) {
  try {
    ...
    const alivePlayers = allPlayers.filter(
      (p) => p.isAlive && p.id !== victimId,
    );
    const alivePlayersMap = Object.fromEntries(
...
function findNewTargetForPlayerFast(
  assassin,
  alivePlayers,
  victimData,
  assassinCountMap,
  ...
  const potentialTargets = alivePlayers.filter(
    (p) => p.id !== assassin.id && p.studentClass !== playerClass,
  );
  if (potentialTargets.length === 0) return null;

  // Layer 1
  if (victimData.targetId) {
    const victimTarget = potentialTargets.find(
      (p) => p.id === victimData.targetId,
    );
    if (victimTarget) return victimTarget.id;
  }

  // Layer 2
  const noAssassinTargets = potentialTargets.filter(
    (p) => (assassinCountMap[p.id] || 0) === 0,
  );
