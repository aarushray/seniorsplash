export const badges = [
  ...
  },
  {
    ...
    icon: '🌑',
    title: 'Erebus Rising',
    description: 'You\'ve tasted the shadows twice. Darkness starts to recognize you.',
    trigger: 'kill_count',
    requirement: 2,
    category: 'milestone'
  },
  ...
  },
  {
    ...
    description: 'In the lawless chaos, you thrived. The underworld opens for you.',
    trigger: 'purge_kill',
    requirement: 1,
    category: 'special'
  },
...
export const checkEligibleBadges = (playerData, actionType, gameData) => {
  ...
  badges.forEach(badge => {
    ...
    switch (badge.trigger) {
      ...
      case 'purge_kill':
        isEligible = actionType === 'purge_kill' && 
...
const checkKillStreak = (playerData, requiredStreak) => {
  ...
  const killsInLastHour = recentKills.filter(kill => 
    new Date(kill.timestamp) > oneHourAgo
  );
