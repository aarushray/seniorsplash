...
export const reassignTargetsAfterPurge = async () => {
  ...
  for (const assassinClass of classes) {
    ...
    const potentialTargets = alivePlayers.filter(
      (player) => player.studentClass !== assassinClass,
    );
    ...
    for (const assassin of assassins) {
      ...
      const targetsWithNoAssassins = potentialTargets.filter(
        (target) =>
      ...
      if (targetsWithNoAssassins.length > 0) {
        ...
        if (validTargets.length > 0) {
          ...
          validTargets.sort(
            (a, b) => targetAssignmentCount[a.id] - targetAssignmentCount[b.id],
          );
          ...
          console.warn(
            `Forced assignment for ${assassin.fullName} - mutual targeting may occur`,
          );
        }
      }
      ...
      if (bestTarget) {
        ...
        console.log(
          `${assassin.fullName} (${assassin.studentClass}) → ${bestTarget.fullName} (${bestTarget.studentClass})`,
        );
      } else {
