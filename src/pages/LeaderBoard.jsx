...
const LeaderBoard = () => {
      ...
      (snapshot) => {
        try {
          ...
          const sortedClasses = Object.values(classSummary).sort((a, b) => {
            if (b.alivePlayers !== a.alivePlayers)
  ...
  const hasGlobalTopKiller = useCallback(
    (classData) => {
      return (
        globalTopKiller &&
        classData.players.some((player) => player.id === globalTopKiller.id)
      );
    },
    [globalTopKiller],
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-slate-200 font-mono">
      ...
      <main className="max-w-6xl mx-auto px-6 pb-10">
          ...
          {classStats.map((classData, index) => (
            ...
            >
              ...
              <div className="absolute -top-4 left-4 z-10">
                <div
                  className={`w-14 h-14 rounded-full bg-gradient-to-r ${getClassColor(index)} flex items-center justify-center text-white font-extrabold text-xl shadow-md ${index === 0 ? "animate-pulse" : ""}`}
                >
                  #{index + 1}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-6 pl-20">
                <div className="flex items-center gap-3">
                  ...
                  <motion.div
                    animate={{
                      rotate: expandedClass === classData.className ? 180 : 0,
                    }}
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold text-green-400">
                    {classData.totalPlayers > 0
                      ? Math.round(
                          (classData.alivePlayers / classData.totalPlayers) *
                            100,
                        )
                      : 0}
                    %
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard
                  icon="👥"
                  value={classData.totalPlayers}
                  label="Total Players"
                  color="blue"
                />
                <StatCard
                  icon="🫀"
                  value={classData.alivePlayers}
                  label="Alive"
                  color="green"
                />
                <StatCard
                  icon="💣"
                  value={classData.totalKills}
                  label="Eliminations"
                  color="red"
                />
              ...
              <div className="mt-6">
                ...
                <div className="w-full bg-gray-800 rounded-full h-3">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${classData.totalPlayers > 0 ? (classData.alivePlayers / classData.totalPlayers) * 100 : 0}%`,
                    }}
                      ...
                      {classData.players
                        ...
                        .map((player, playerIndex) => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-3 h-3 rounded-full ${player.isAlive ? "bg-green-400" : "bg-red-400"} animate-pulse`}
                              />
        ...
        >
          <h3 className="text-3xl font-extrabold text-pink-400 mb-4">
            🌌 SYSTEM WIDE TOTALS
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <TotalStat
              value={classStats.reduce((sum, cls) => sum + cls.totalPlayers, 0)}
              label="Total Players"
              color="blue"
            />
            <TotalStat
              value={classStats.reduce((sum, cls) => sum + cls.alivePlayers, 0)}
              label="Alive"
              color="green"
            />
            <TotalStat
              value={classStats.reduce((sum, cls) => sum + cls.totalKills, 0)}
              label="Kills"
              color="red"
            />
          </div>
  ...
  return (
    ...
    >
      ...
      {hasMVP ? (
        <>
          <div
            className="text-lg font-bold text-purple-300 truncate"
            title={classData.topKiller}
          >
            {classData.topKiller}
          </div>
          <div className="text-sm text-purple-400">
            {classData.topKillerKills} kills
          </div>
        </>
      ) : (
        <>
