  ...
  const handleMediaChange = (e) => {
    ...
    if (file) {
      ...
      if (file.size > maxSize) {
        setError(
          `File too large. Max size: ${file.type.startsWith("video/") ? "50MB for videos" : "10MB for images"}`,
        );
        return;
      }
    ...
    try {
      ...
      if (!isPurgeMode) {
        ...
        if (!submitterData.targetId) {
          setError(
            "You do not have an assigned target. Please contact an admin.",
          );
          setIsLoading(false);
          return;
        }
        ...
        if (!targetSnap.exists()) {
          setError(
            "Your assigned target was not found. Please contact an admin.",
          );
          setIsLoading(false);
          return;
        }
        ...
        if (
          assignedTargetName.toLowerCase() !== trimmedTargetName.toLowerCase()
        ) {
          setError(
            `It is not Purge Mode. You may only assassinate your assigned target: ${assignedTargetName}`,
          );
          setIsLoading(false);
          return;
        }
        ...
        if (!targetData.isAlive) {
          setError(
            `${assignedTargetName} has already been eliminated. Please refresh your dashboard for a new target.`,
          );
          setIsLoading(false);
          return;
        }
        ...
        const playerQuery = query(
          playersRef,
          where("fullName", "==", trimmedTargetName),
        );
        ...
        if (playerSnapshot.size > 1) {
          setError(
            `Multiple players found with name: ${trimmedTargetName}. Please contact an admin.`,
          );
          setIsLoading(false);
          return;
        }
          ...
          <div className="glass-card rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            >
              ...
              <p className="text-gray-400 text-lg">
                Document your victory with photo/video evidence
              </p>
            </motion.div>
                        ...
                        {alivePlayers.length > 0 ? (
                          alivePlayers.map((player) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                handlePlayerSelect(player.fullName)
                              }
